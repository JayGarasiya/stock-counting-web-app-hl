...
using Nop.Data;
using Nop.Plugin.Misc.CountingSequence.Components;
...
{
    /// <summary>
    /// Represents counting sequence plugin
    /// </summary>
    public class CountingSequencePlugin : BasePlugin, IMiscPlugin, IWidgetPlugin
    {
        #region Fields
...
");
        ...
        {
            return $"{_webHelper.GetStoreLocation()}Admin/CountingSequence/Configure";
        }
        ...
        {
            return typeof(CountingSequenceQrButtonViewComponent);
        }
        ...
        {
            var settings = new CountingSequenceSettings
            {
                Enabled = false,
                SaleTypeOrder = 0,
                PalletCountingOrder =  PalletCounting.OldToNew,
                Level = SequenceLevel.TopToBottom,
                Position = SequencePositions.LeftToRight
            };

            await _settingService.SaveSettingAsync(settings);

            var existingRole = await _customerService.GetCustomerRoleBySystemNameAsync(CountingSequenceDefaults.CustomerRole);
            ...
            if (existingRole == null)
            {
                await _customerService.InsertCustomerRoleAsync(new CustomerRole
                {
                    ...
                    SystemName = CountingSequenceDefaults.CustomerRole,
                    Active = true,
                    IsSystemRole = false
                });
            }
            ...
            var existingPermission = allPermissions.FirstOrDefault(p => p.SystemName == CountingSequenceDefaults.CountingSequnceTabList);
            ...
            await _localizationService.AddOrUpdateLocaleResourceAsync(new Dictionary<string, string>
            {
                ...
                ["Plugins.Misc.CountingSequence.Fields.CountComplete"] = "The stock quantity has been added by the counting complete #{0}",
                ...
                ["Plugins.Misc.CountingSequence.ShipmentTransitItem.Units"] = "Units",
                ["Plugins.Misc.CountingSequence.ShipmentTransitItem.Cases"] = "Cases",
        ...
        {
            await _settingService.DeleteSettingAsync<CountingSequenceSettings>();

            var role = await _customerService.GetCustomerRoleBySystemNameAsync(CountingSequenceDefaults.CustomerRole);
            if (role != null)
                await _customerService.DeleteCustomerRoleAsync(role);

            await _localizationService.DeleteLocaleResourcesAsync("Plugins.Misc.CountingSequence");
            await _localizationService.DeleteLocaleResourcesAsync("Enum.Nop.Plugin.Misc.CountingSequence");
